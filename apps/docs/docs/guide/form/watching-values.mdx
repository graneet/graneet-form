...
```tsx
import { useOnBlurValues, useFormContext } from 'graneet-form';
...
function ValidationSummary() {
  ...
  const { name, email, password } = useOnBlurValues(form, ['name', 'email', 'password']);
...
function AutoSaveStatus() {
  ...
  const { title, content } = useOnChangeValues(form, ['title', 'content']);
...
function AddressForm() {
  ...
  const { zipCode } = useOnBlurValues(form, ['zipCode']);
...
const { searchTerm } = useOnChangeValues(form, ['searchTerm']);

// For validation, summaries, auto-save (less frequent updates)
const { title, content } = useOnBlurValues(form, ['title', 'content']);
```
...
function ExpensiveFormWatcher() {
  const form = useFormContext<FormData>();
  const { complexField } = useOnChangeValues(form, ['complexField']);
...
function ComprehensiveFormWatcher() {
  ...
  const { title, description, tags } = useOnBlurValues(form, ['title', 'description', 'tags']);
...
function useFormProgress<T>(form: FormContextApi<T>, requiredFields: (keyof T)[]) {
  const watchedValues = useOnBlurValues(form, requiredFields);
...
Global watching (`useOnChangeValues(form, undefined)`) causes components to re-render on every form change. Use sparingly and only when necessary.
:::
