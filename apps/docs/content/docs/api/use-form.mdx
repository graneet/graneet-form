---
title: useForm
description: Main hook for creating and managing form instances
---

# useForm Hook

The `useForm` hook is the main entry point for creating form instances in Graneet Form. It provides a complete form management solution with optimized performance and granular subscriptions.

## Usage

```tsx
import { useForm } from 'graneet-form';

const form = useForm<FormDataType>(options);
```

## Parameters

<AutoTypeTable path="../../packages/graneet-form/src/form/hooks/use-form.ts" name="UseFormOptions" />

## Returns

The hook returns a form context API object:

<AutoTypeTable path="../../packages/graneet-form/src/form/contexts/form-context.ts" name="FormContextApi" />

## Examples

### Basic Form

```tsx
interface UserForm {
  name: string;
  email: string;
}

function MyComponent() {
  const form = useForm<UserForm>({
    defaultValues: {
      name: '',
      email: '',
    },
  });

  return <Form form={form}>{/* Your form fields */}</Form>;
}
```

### Form with onUpdateAfterBlur

```tsx
const form = useForm<UserForm>({
  defaultValues: {
    name: '',
    email: '',
  },
  onUpdateAfterBlur: async (fieldName, value, data, { getFormValues, setFormValues }) => {
    if (fieldName === 'email' && value) {
      // Auto-populate based on email
      const userInfo = await fetchUserByEmail(value);
      if (userInfo.name) {
        setFormValues({ name: userInfo.name });
      }
    }
  },
});
```

### Dynamic Default Values

```tsx
const form = useForm<UserForm>({
  defaultValues: () => ({
    name: user?.name || '',
    email: user?.email || '',
    timestamp: Date.now(),
  }),
});
```

## Form Context API Methods

### getFormValues()

Retrieves all current form field values. Nested objects are reconstructed.

```tsx
const currentValues = form.getFormValues();
// Returns: { name: 'John', email: 'john@example.com' }
// With nested paths: { user: { address: { city: 'Paris' } } }
```

### setFormValues(newValues)

Updates multiple form field values at once. Accepts a partial nested object (`DeepPartial<T>`), so deep leaves can be
updated without their siblings. See [Nested Object Paths](/docs/form/nested-paths).

```tsx
form.setFormValues({
  name: 'Jane Doe',
  email: 'jane@example.com',
});

// Partial nested update
form.setFormValues({
  user: { address: { city: 'Lyon' } },
});
```

### resetForm()

Resets all form fields to their default values.

```tsx
form.resetForm();
```

### handleSubmit(callback)

Registers a submission handler that runs only when the whole form is valid. `handleSubmit` returns a function that you pass to the `onSubmit` prop of the [`Form`](/docs/api/components) component — `Form` renders the underlying `<form>` element and triggers validation on submit, calling your callback only if every field passes.

```tsx
function MyForm() {
  const form = useForm<FormData>();

  const onSubmit = (formData: FormData) => {
    console.log('Valid form data:', formData);
  };

  return (
    <Form form={form} onSubmit={form.handleSubmit(onSubmit)}>
      {/* fields */}
      <button type="submit">Submit</button>
    </Form>
  );
}
```

## Performance Notes

- Form state is managed with refs for optimal performance
- Only components that subscribe to specific fields re-render when those fields change
- Global form watchers are debounced to prevent excessive updates
- Field registration/unregistration is handled automatically
