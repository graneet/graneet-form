...
function WizardNavigation() {
  const { steps, currentStep, goNext, goPrevious, goBackTo, isFirstStep, isLastStep, isStepReady } =
  ...
  return (
    <div className="wizard-navigation">
      ...
      <div className="navigation-buttons">
        <button onClick={goPrevious} disabled={isFirstStep} className="btn-secondary">
          {isFirstStep ? 'Cancel' : 'Previous'}
        </button>

        <button onClick={goNext} disabled={!isStepReady} className={isStepReady ? 'btn-primary' : 'btn-disabled'}>
      ...
      {steps.map((stepName, index) => {
        ...
        return (
          <span key={stepName} className="breadcrumb-item">
            ...
            {isAccessible ? (
              <button onClick={() => goBackTo(stepName)} className={`breadcrumb-link ${isCurrent ? 'current' : ''}`}>
                {stepName}
              </button>
            ) : (
...
function StepValidationStatus() {
  ...
  const getStatusMessage = () => {
    switch (stepStatus) {
      ...
      default:
        return `📝 Complete ${currentStep} step to continue`; // 'undetermined'
    }
  };

  return <div className={`step-status status-${stepStatus}`}>{getStatusMessage()}</div>;
}
```
