...
function RegistrationWizard() {
  const steps: Steps<WizardData> = [{ name: 'userInfo' }, { name: 'preferences' }, { name: 'confirmation' }];
...
function WizardContent() {
  ...
  return <div className="wizard-content">{renderStep()}</div>;
}
```
...
function ConditionalWizard() {
  ...
  const steps: Steps<WizardData> = useMemo(() => {
    const baseSteps = [{ name: 'userInfo' }, { name: 'preferences' }];
