...
`names` accepts nested [field paths](/docs/form/nested-paths) (e.g. `'user.address.city'`). The result is always a
**reconstructed nested object**, and watching a parent path (`'user'`) re-renders whenever any descendant changes.
...
function CitySummary() {
  ...
  // Re-renders on any change under `user`; receives the rebuilt subtree
...
function ConditionalFields() {
  ...
  return (
    <div>
      ...
      {accountType === 'personal' && <Field name="dateOfBirth" />}
    </div>
  );
}
```
