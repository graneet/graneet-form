...
const { form } = useStepForm<WizardValues, StepName>(options);
```
...
function PreferencesStep() {
  ...
  return (
    <Form form={form}>
      <Field name="theme">
        <Rule validationFn={(value) => ['light', 'dark'].includes(value)} message="Please select a valid theme" />
      </Field>
...
Pass `defaultValues` to seed the step's fields. (The legacy `initFormValues` method was removed — use `defaultValues` instead.)
