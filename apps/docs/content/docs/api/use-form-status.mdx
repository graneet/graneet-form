...
function SubmitButton() {
  ...
  return (
    <button type="submit" disabled={!isValid} className={isValid ? 'enabled' : 'disabled'}>
      Submit Form
    </button>
  );
}
...
- `'undetermined'` - Initial state, no validation rules, or async validation still running
