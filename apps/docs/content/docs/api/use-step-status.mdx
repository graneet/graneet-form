...
`useStepStatus` returns a `ValidationStatus` — the same three values used elsewhere in the library:
  ...
  const getButtonClass = () => {
    switch (stepStatus) {
      ...
      case 'invalid':
        return 'btn-disabled';
      default:
        return 'btn-default'; // 'undetermined'
    }
  };

  return (
    <button onClick={goNext} disabled={stepStatus !== 'valid'} className={getButtonClass()}>
      {getButtonText()}
    </button>
  );
}
...
function StepStatusIndicator() {
  ...
  const getStatusMessage = () => {
    switch (stepStatus) {
      ...
      default:
        return 'Fill out the required fields'; // 'undetermined'
    }
  };
  ...
  return (
    <div className="progress-bar">
      {steps.map((stepName, index) => {
        ...
        return (
          <div key={stepName} className={`progress-step ${status}`}>
...
function FormActions() {
  ...
  return (
    <div className="form-actions">
      <button onClick={goPrevious} className="btn-secondary">
...
function MyWizardStep() {
  ...
  return (
    <div>
      <Form form={form}>{/* Your form fields */}</Form>
