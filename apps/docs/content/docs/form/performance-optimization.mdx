...
description: Learn how to optimize graneet-form for maximum performance in large and complex applications
---
  ...
  return (
    <div className="virtualized-container">
      ...
      {Array.from({ length: visibleRange.end - visibleRange.start }).map((_, index) => {
        ...
        return <Field key={fieldIndex} name={`field_${fieldIndex}`} render={/* ... */} />;
      })}
    </div>
  );
}
...
<Field name="email" render={/* ... */}>
  ...
  <Rule validationFn={checkEmailAvailability} message="Email already exists" isDebounced={true} />
</Field>
```
...
function AsyncValidatedField() {
  ...
  return (
    <Field name="username" render={/* ... */}>
      <Rule validationFn={debouncedValidation} message="Username validation failed" isDebounced={true} />
    </Field>
  );
}
```
...
const MemoizedTextField = React.memo(
  <T extends FieldValues, K extends keyof T>({
    name,
    label,
    placeholder,
  }: {
...
// Usage
<MemoizedTextField name="email" label="Email Address" placeholder="Enter email" />;
```

#### Memoize Form Sections

```tsx
const MemoizedFormSection = React.memo(({ title, children }: { title: string; children: React.ReactNode }) => (
...
function SafeAsyncField() {
  ...
  return (
    <Field name="asyncField" render={/* ... */}>
      <Rule validationFn={handleAsyncValidation} message="Validation failed" isDebounced={true} />
    </Field>
  );
}
```
