...
description: Complete guide to implementing robust form validation with graneet-form's rule-based validation system
---
...
>
  <Rule validationFn={(value) => !!value && value.includes('@')} message="Please enter a valid email address" />
  <Rule validationFn={(value) => !value || value.length <= 100} message="Email must be less than 100 characters" />
</Field>
```
...
<Rule validationFn={isRequired} message="This field is required" />;
```
...
```tsx
<Rule validationFn={(age: number) => age >= 18 && age <= 120} message="Age must be between 18 and 120" />
```
...
```tsx
<Field<FormData, 'username'> name="username" render={/* render function */}>
  <Rule validationFn={(value) => !!value && value.length > 0} message="Username is required" />
  <Rule validationFn={(value) => !value || value.length >= 3} message="Username must be at least 3 characters" />
  <Rule validationFn={(value) => !value || value.length <= 20} message="Username must be less than 20 characters" />
  <Rule
...
function PasswordConfirmField() {
  ...
  return (
    <Field<FormData, 'confirmPassword'> name="confirmPassword" render={/* render function */}>
      <Rule
...
- `undetermined` - Not yet validated, or an async rule is still running

<Callout type="info">
  There is no separate `pending` status. While a debounced or async rule is resolving, the field stays `undetermined`
  until it settles to `valid` or `invalid`.
</Callout>
...
<Field
  name="email"
  render={(fieldProps, fieldState) => {
    const { isTouched, validationStatus } = fieldState;
    ...
    return (
      <div className="field-container">
        <input
          className={`
            input 
            ${isTouched && validationStatus.status === 'invalid' ? 'error' : ''}
            ${isTouched && validationStatus.status === 'valid' ? 'success' : ''}
          `}
          {...fieldProps}
        />

        {/* Show error message */}
        {isTouched && validationStatus.status === 'invalid' && (
          <span className="validation-error">❌ {validationStatus.message}</span>
        )}

        {/* Show success indicator */}
        {isTouched && validationStatus.status === 'valid' && <span className="validation-success">✅ Valid</span>}
      </div>
    );
  }}
>
...
function ValidationSummary() {
  ...
  const hasErrors = Object.values(allValidations).some((validation) => validation?.status === 'invalid');
    ...
    <div className="validation-summary">
      ...
      {hasErrors && (
        <div className="error-summary">
          ...
          <ul>
            {Object.entries(allValidations).map(
              ([field, validation]) =>
      ...
      <div className="field-status">
        <div className={`status ${email?.status ?? 'undetermined'}`}>Email: {email?.status ?? 'undetermined'}</div>
        <div className={`status ${password?.status ?? 'undetermined'}`}>
  ...
  return (
    <div className="submit-section">
      <div className="form-status">
        Status: <span className={formStatus}>{formStatus}</span>
      </div>

      <button type="submit" disabled={!isValid} className={`submit-btn ${isValid ? 'enabled' : 'disabled'}`}>
        Submit Form
      </button>
    </div>
  );
...
export function LengthRule({ min, max, message }: LengthRuleOptions) {
  ...
  return <Rule validationFn={validationFn} message={message || defaultMessage} />;
}
...
<Field name="email" render={/* ... */}>
  ...
  <Rule validationFn={checkEmailExists} message="Email already exists" isDebounced={true} />
</Field>
```
  ...
  return (
    <Field name="taxId" render={/* ... */}>
      {accountType === 'business' && (
        <Rule validationFn={(value) => !!value} message="Tax ID is required for business accounts" />
      )}
    </Field>
  );
}
