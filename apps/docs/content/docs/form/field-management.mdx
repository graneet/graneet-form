...
<Field<FormData, 'username'> name="username" render={(fieldProps, fieldState) => <input {...fieldProps} />} />
```
...
function FieldLifecycleExample() {
  ...
  return (
    <Form form={form}>
      <button onClick={() => setShowField(!showField)}>{showField ? 'Hide' : 'Show'} Dynamic Field</button>

      {showField && (
        <Field<FormData, 'dynamicField'>
          name="dynamicField"
          render={(fieldProps) => <input placeholder="This field registers when mounted" {...fieldProps} />}
        />
      )}
...
```tsx
interface FieldProps<T extends FieldValues, K extends FieldPath<T>> {
  name: K; // Field name — supports nested paths like "user.address.city"
  ...
  defaultValue?: FieldPathValue<T, K>; // Default value for this field
}
```

`name` accepts any [`FieldPath<T>`](/docs/api/types#fieldpath) — every top-level key plus any nested object path. See
[Nested Object Paths](/docs/form/nested-paths) for details.
...
```tsx
interface FieldRenderProps<T, K extends FieldPath<T>> {
  name: K; // Field name
  value: FieldPathValue<T, K> | undefined; // Current field value
  onChange(value: FieldPathValue<T, K>): void; // Value change handler
...
interface FieldRenderState {
  ...
  isPristine: boolean; // equivalent to !isDirty
  isTouched: boolean; // true after the user has focused then blurred the field
  isDirty: boolean; // true after the user has changed the field value
  validationStatus: {
    status: 'valid' | 'invalid' | 'undetermined'; // 'undetermined' while async rules are still running
  ...
  render={(fieldProps, fieldState) => (
    <div className="field-group">
      ...
      <input
        ...
        className={fieldState.isTouched && fieldState.validationStatus.status === 'invalid' ? 'error' : ''}
      />
      {fieldState.isTouched && fieldState.validationStatus.status === 'invalid' && (
...
<Field<FormData, 'price'>
  ...
  render={(fieldProps, fieldState) => (
    <div className="field-group">
      ...
      {fieldProps.value && <div className="formatted-value">Formatted: ${fieldProps.value.toFixed(2)}</div>}
    </div>
  )}
>
...
<Field<FormData, 'description'>
  ...
  render={(fieldProps, fieldState) => (
    <div className="field-group">
      ...
      />
      {/* Character counter */}
      <div className="char-counter">{(fieldProps.value || '').length} / 500 characters</div>
      {fieldState.isTouched && fieldState.validationStatus.status === 'invalid' && (
        <span className="error-message">{fieldState.validationStatus.message}</span>
      )}
    </div>
    ...
    <div className="field-group">
      ...
      {fieldProps.value && (
        <div className="file-preview">
          <img src={URL.createObjectURL(fieldProps.value)} alt="Preview" style={{ maxWidth: 200, maxHeight: 200 }} />
          <button type="button" onClick={() => fieldProps.onChange(undefined)}>
            Remove
          </button>
        </div>
      )}
...
function TagsField() {
  return (
    ...
    >
      <Rule validationFn={(tags) => tags && tags.length > 0} message="At least one tag is required" />
    </Field>
  );
}
```
...
> The example below manages a whole object under a single field. If you prefer to bind each nested property to its own
> input, use [dotted field names](/docs/form/nested-paths) (e.g. `name="address.city"`) instead.
...
function AddressField() {
  return (
    ...
    >
      <Rule
        validationFn={(address) => address && address.street && address.city && address.zipCode}
...
<Field<FormData, 'name'> name="name" defaultValue="Field Default" render={(fieldProps) => <input {...fieldProps} />} />;
```
...
function SmartEmailField() {
  ...
  return (
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (e.target.value.includes('@')) {
                ...
                setSuggestions(
                  [`${username}@gmail.com`, `${username}@hotmail.com`, `${username}@yahoo.com`].filter(
                    (suggestion) => suggestion !== e.target.value,
                  ),
                );
              }
            }}
    ...
    >
      <Rule validationFn={(email) => !!email && email.includes('@')} message="Please enter a valid email" />
    </Field>
  );
}
```
...
function GenericField<T extends FieldValues, K extends keyof T>({
  ...
  return (
    ...
    >
      {required && <Rule validationFn={(value) => !!value} message={`${label} is required`} />}
      {children}
    </Field>
  );
}
...
// Usage
<GenericField<FormData, 'name'> name="name" label="Full Name" placeholder="Enter your full name" required={true} />;
```
...
```tsx
const MemoizedField = React.memo(<T extends FieldValues, K extends keyof T>({ name, ...props }: FieldProps<T, K>) => {
  return <Field<T, K> name={name} {...props} />;
});
