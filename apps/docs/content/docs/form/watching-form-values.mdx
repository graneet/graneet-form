...
`useFieldsWatch` accepts a `mode` option that controls _when_ updates are published: `onChange` (the default) or `onBlur`.

### `onChange` (default) — Real-time Updates
...
### `onBlur` — Update on Blur

Updates only when fields lose focus (after the user finishes editing).
...
## Watching Nested Paths

`useFieldsWatch` accepts nested [field paths](/docs/form/nested-paths). Watching a parent path re-renders whenever any
descendant changes, and the result is a reconstructed nested object.

```tsx
function CitySummary() {
  const form = useFormContext<{ user: { address: { city: string } } }>();

  // Notified on any change under `user` (e.g. user.address.city)
...
function ComprehensiveFormWatcher() {
  ...
  return (
    <div>
      ...
      {process.env.NODE_ENV === 'development' && <pre>{JSON.stringify(allValues, null, 2)}</pre>}
    </div>
  );
}
```
...
<Callout type="warn" title="Performance impact">
  Global watching (`useFieldsWatch(form, undefined)`) causes the component to re-render on every form change. Use it
  sparingly and only when you genuinely need every field.
</Callout>

<Callout type="info" title="Best practice">
  Always prefer specific field watching over global watching. If you need many fields, consider splitting your component
  or using multiple targeted watchers.
</Callout>
