...
const steps = [{ name: 'userInfo' as const }, { name: 'preferences' as const }];
...
function WizardComponent() {
  ...
  return <WizardContext.Provider value={wizard}>{/* Wizard steps */}</WizardContext.Provider>;
}
...
function StepNavigation() {
  ...
  return (
    <button onClick={goNext} disabled={!isStepReady || stepStatus !== 'valid'}>
      Next Step
    </button>
  );
}
...
```tsx
import { useWizard, useStepForm, WizardContext, Step, Form, Field } from 'graneet-form';
...
function WizardNavigation() {
  const { goNext, goPrevious, isFirstStep, isLastStep, isStepReady, currentStep } = useWizardContext();
  ...
  return (
    <div>
      ...
      <div>
        {!isFirstStep && <button onClick={goPrevious}>Previous</button>}
        <button onClick={goNext} disabled={!isStepReady}>
