...
<Callout type="info">
  Check out our CTO's POC from several years ago: [React Hook Form Multi-step
  Discussion](https://github.com/orgs/react-hook-form/discussions/1979)
</Callout>
...
```tsx
// ❌ Other libraries: errors live in one shared object,
// so any error change re-renders every component reading it.
setError('field1', { message: 'Error' }); // Re-renders 1000+ components

// ✅ Graneet Form: each field's validation status is an independent subscription.
// When `field1` becomes invalid, only components watching `field1` re-render.
const { field1 } = useValidations(form, ['field1']); // Re-renders 1 component
```
...
const wizard = useWizard([{ name: 'step1' }, { name: 'step2' }, { name: 'step3' }], (wizardValues) =>
  submitRegistration(wizardValues),
); // Automatic state management and persistence across steps
```
